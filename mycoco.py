...
def iter_captions(idlists, cats, batch=1):
    ...
    Returns an infinite iterator (do not convert to list!) that returns tuples
    (captions, categories)
...
def iter_captions_cats(idlists, cats, batch=1):
    '''
    Obtains the corresponding captions from multiple COCO id lists alongside all
    associated image captions per image.
    Randomizes the order.  
    Returns an infinite iterator (do not convert to list!) that returns tuples
    (captions, categories) as parallel lists at size of batch.
    '''
    if not capcoco:
        raise ValueError
    if batch < 1:
...
def list_captions_cats(idlists, cats):
    '''
    Obtains the corresponding captions from multiple COCO id lists alongside all
    associated image captions per image.
    Randomizes the order.
    Returns two parallel lists:
        - List of captions split in words.
        - List of lists of categories for each caption.
    '''
    if not capcoco:
        raise ValueError
    ...
    for p in randomlist:
        ...
        for ann in anns:
            imgid = ann['image_id']
            cats = get_cats_for_img(imgid)
            captions.append(ann['caption'].split())
            categories.append(cats)
    return captions, categories


def iter_images(idlists, cats, size=(200,200), batch=1):
    '''
    Obtains the corresponding image data as numpy array from multiple COCO id
    lists.
    Returns an infinite iterator (do not convert to list!) that returns tuples
    (images, categories)
    as parallel lists at size of batch.
